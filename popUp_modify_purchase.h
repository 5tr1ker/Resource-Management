...
namespace comonResourceManagement {
	...
	/// <summary>
	/// popUp_modify_purchase에 대한 요약입니다.
	/// </summary>
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->label1->Text = L"수정할 가격을 입력해주세요.";
			// 
			// popUp_modify_purchase
			// 
...
private: System::Void submit_button_Click(System::Object^ sender, System::EventArgs^ e) {
	...
	if (data.size() <= 0) {
		...
		string query = "UPDATE `comon`.`resource_details` SET `price` = '" + data + "원' WHERE (`ulid` = '" + string(ulid) + "');";
		int result = rm.updateResource(query);

		string contents = rd.price + "->" + data;

		rm.createHistory("수정", "가격", contents, etc);
